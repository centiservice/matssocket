  ...
  group('MatsSocket integration tests, basics', () {
    ...
    group('simple sends', () {
      ...
      test('Should have a promise that resolves when received', () async {
        await matsSocket.send('Test.single', 'SEND_${matsSocket.randomId(6)}', {'string': 'The String', 'number': math.pi});
      });

      test('Send to non-existing endpoint should NACK from Server and reject Promise', () async {
        var promise =
            matsSocket.send('NON EXISTING!', 'SEND_NonExisting${matsSocket.randomId(6)}', {'string': 'The String', 'number': math.pi});
    ...
    group('request', () {
      ...
      test('Request should resolve Promise', () async {
        await matsSocket
            .request('Test.single', 'REQUEST-with-Promise_${matsSocket.randomId(6)}', {'string': 'Request String', 'number': math.e});
      });

      test('Request should invoke both the ack callback and resolve Promise', () async {
        var received = false;
        await matsSocket.request('Test.single', 'REQUEST-with-Promise-and-receivedCallback_${matsSocket.randomId(6)}', {},
      ...
      test('Request to non-existing endpoint should NACK from Server, invoking receivedCallback and reject Promise',
          () async {
        expect(
            matsSocket.request('NON EXISTING!', 'REQUEST_NonExisting${matsSocket.randomId(6)}', {}, receivedCallback: (event) {
              standardStateAssert();
            }),
    ...
    group('requestReplyTo', () {
      ...
      test('Should reply to our own endpoint', () async {
        ...
        await matsSocket.requestReplyTo('Test.single', 'REQUEST-with-ReplyTo_${matsSocket.randomId(6)}',
      ...
      test('Should reply to our own endpoint with our correlation data', () async {
        var correlationInformation = matsSocket.randomId(5);
        var terminator = matsSocket.terminator('ClientSide.testTerminator');
        await matsSocket.requestReplyTo('Test.single', 'REQUEST-with-ReplyTo_withCorrelationInfo_${matsSocket.randomId(6)}',
            {'string': 'Request String', 'number': math.e}, 'ClientSide.testTerminator',
    ...
    group('timeout on request', () {
      ...
      test('requestReplyTo with timeout earlier than slow endpoint', () async {
        var correlationInformation = matsSocket.randomCId(100);
        var testCompleter = Completer();
        ...
        void test() {
          ...
          matsSocket
              .requestReplyTo(
              'Test.slow', 'Timeout_requestReplyTo_${matsSocket.randomId(6)}', req,
              'Test.terminator',
              correlationInformation: correlationInformation,
    ...
    group('pipeline', () {
      ...
      test('Pipeline should reply to our specified Terminator', () async {
        ...
        var req1 = matsSocket.requestReplyTo(
            'Test.single',
            'REQUEST-with-ReplyTo_Pipeline_msg1of3_${matsSocket.randomId(6)}',
            {'string': 'Messge 1', 'number': 100.001, 'requestTimestamp': DateTime.now().millisecondsSinceEpoch},
            'ClientSide.testEndpoint',
            correlationInformation: 'pipeline_msg1of3_${matsSocket.randomId(10)}');
        var req2 = matsSocket.requestReplyTo(
            'Test.single',
            'REQUEST-with-ReplyTo_Pipeline_msg2of3_${matsSocket.randomId(6)}',
            {'string': 'Message 2', 'number': 200.002, 'requestTimestamp': DateTime.now().millisecondsSinceEpoch},
            'ClientSide.testEndpoint',
            correlationInformation: 'pipeline_msg2of3_${matsSocket.randomId(10)}');
        var req3 = matsSocket.requestReplyTo(
            'Test.single',
            'REQUEST-with-ReplyTo_Pipeline_msg3of3_${matsSocket.randomId(6)}',
            {'string': 'Message 3', 'number': 300.003, 'requestTimestamp': DateTime.now().millisecondsSinceEpoch},
            'ClientSide.testEndpoint',
            correlationInformation: 'pipeline_msg3of3_${matsSocket.randomId(10)}');
        matsSocket.flush();
    ...
    group('requests handled in IncomingAuthorizationAndAdapter', () {
      ...
      test(
        ...
        var promise = matsSocket.request(
            'Test.ignoreInIncomingHandler', 'REQUEST_ignored_in_incomingHandler${matsSocket.randomId(6)}', {},
            receivedCallback: (event) => received = true);
      ...
      test('context.deny() should NACK (and thus reject Promise)', () async {
        var received = false;
        var promise = matsSocket.request('Test.denyInIncomingHandler', 'REQUEST_denied_in_incomingHandler${matsSocket.randomId(6)}', {},
            receivedCallback: (event) => received = true);
        await promise.catchError((event) {
          standardStateAssert();
          expect(received, isTrue, reason: 'The received-callback should have been invoked.');
      ...
      test('context.resolve(..) should ACK received, and RESOLVE the Promise.', () async {
        var received = false;
        var promise = matsSocket.request(
            'Test.resolveInIncomingHandler', 'REQUEST_resolved_in_incomingHandler${matsSocket.randomId(6)}', {},
            receivedCallback: (event) => received = true);
      ...
      test('context.reject(..) should ACK received, and REJECT the Promise', () async {
        var received = false;
        var promise = matsSocket.request(
            'Test.rejectInIncomingHandler', 'REQUEST_rejected_in_incomingHandler${matsSocket.randomId(6)}', {},
            receivedCallback: (event) => received = true);
        await promise.catchError((event) {
          standardStateAssert();
          expect(received, isTrue, reason: 'The received-callback should have been invoked.');
      ...
      test('Exception in incomingAdapter should NACK (and thus reject Promise)', () async {
        var received = false;
        var promise = matsSocket.request(
            'Test.throwsInIncomingHandler', 'REQUEST_throws_in_incomingHandler${matsSocket.randomId(6)}', {},
            receivedCallback: (event) => received = true);
        await promise.catchError((event) {
          standardStateAssert();
          expect(received, isTrue, reason: 'The received-callback should have been invoked.');
    ...
    group('sends handled in IncomingAuthorizationAndAdapter', () {
      ...
      test('Ignored (handler did nothing) should ACK when SEND (thus resolve Promise)', () async {
        await matsSocket.send('Test.ignoreInIncomingHandler', 'SEND_ignored_in_incomingHandler${matsSocket.randomId(6)}', {});
        standardStateAssert();
      });

      test('context.deny() should NACK (reject Promise)', () async {
        var promise = matsSocket.send('Test.denyInIncomingHandler', 'SEND_denied_in_incomingHandler${matsSocket.randomId(6)}', {});
        await promise.catchError((receivedEvent) {
          standardStateAssert();
          return receivedEvent; // Satisfy Dart3
        });
      ...
      test('context.resolve(..) should NACK (reject Promise) since it is not allowed to resolve/reject a send',
          () async {
        var promise = matsSocket.send('Test.resolveInIncomingHandler', 'SEND_resolved_in_incomingHandler${matsSocket.randomId(6)}', {});
        await promise.catchError((receivedEvent) {
          standardStateAssert();
          return receivedEvent; // Satisfy Dart3
        });
      ...
      test('context.reject(..) should NACK (reject Promise) since it is not allowed to resolve/reject a send',
          () async {
        var promise = matsSocket.send('Test.rejectInIncomingHandler', 'SEND_rejected_in_incomingHandler${matsSocket.randomId(6)}', {});
        await promise.catchError((receivedEvent) {
          standardStateAssert();
          return receivedEvent; // Satisfy Dart3
        });
      ...
      test('Exception in incomingAdapter should NACK (reject Promise)', () async {
        var promise = matsSocket.send('Test.throwsInIncomingHandler', 'SEND_throws_in_incomingHandler${matsSocket.randomId(6)}', {});
        await promise.catchError((receivedEvent) {
          return receivedEvent; // Satisfy Dart3
        });
      });
    ...
    group('requests handled in replyAdapter', () {
      ...
      test('Ignored (handler did nothing) should NACK when Request handled in adaptReply(..) (thus nack receivedCallback, and reject Promise)',
        ...
        var received = false;
        var rejected = false;
        await matsSocket.request('Test.ignoreInReplyAdapter', 'REQUEST_ignored_in_replyAdapter${matsSocket.randomId(6)}', {},
      ...
      test('context.resolve(..)', () async {
        var received = false;
        await matsSocket.request('Test.resolveInReplyAdapter', 'REQUEST_resolved_in_replyAdapter${matsSocket.randomId(6)}', {},
            receivedCallback: (event) {
          standardStateAssert();
          received = true;
        });
      ...
      test('context.reject(..)', () async {
        var received = false;
        var rejected = false;
        await matsSocket.request('Test.rejectInReplyAdapter', 'REQUEST_rejected_in_replyAdapter${matsSocket.randomId(6)}', {},
            receivedCallback: (event) {
          standardStateAssert();
          received = true;
        }).catchError((event) {
      ...
      test('Exception in replyAdapter should reject', () async {
        ...
        await matsSocket.request('Test.throwsInReplyAdapter', 'REQUEST_throws_in_replyAdapter${matsSocket.randomId(6)}', {},
            receivedCallback: (event) {
          standardStateAssert();
          received = true;
        }).catchError((event) {
    ...
    group('debug object', () {
      Future<DebugDto?> testDebugOptionsSend({int? debug, String userId = 'enableAllDebugOptions'}) async {
        ...
        var messageEvent = await matsSocket
            .request('Test.single', 'Request_DebugObject_${debug}_${matsSocket.debug}_${matsSocket.randomId(6)}', {},
      ...
      test('When the user is allowed to debug, but do not request any debug (undefined),'
        ...
        ReceivedEvent? receivedEventReceived;
        var messageEvent = await matsSocket.request('Test.single', 'Request_DebugObject_non_requested_${matsSocket.randomId(6)}', {},
            receivedCallback: (receivedEvent) {
          standardStateAssert();
          receivedEventReceived = receivedEvent;
        });
